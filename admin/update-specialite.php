 ...
 <?php
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE `specialites` set `Description` = ? , `Nom`= ? where id = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($description,$nom,$id));
            Database::disconnect();
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM specialites where id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
...
<html lang="en">
  <head>
    ...
    <title>Affiche specialité </title>
  ...
  <div class="panel-default">
    ...
    <div id="collapse1" class="panel-collapse collapse">
      <div class="panel-footer"><a href="authentification.php"> <span class="glyphicon glyphicon-log-out" aria-hidden="true"> </span>Déconnexion</a></div>
        ...
        <form id="form" action ="update-specialite.php" method="post"> 
        <table> 
       <tr>
        <td> <label> Nom specialité </label> </td>
        <td> <input type="text" class="form-control" id="Nom" placeholder="Nom" name="Nom"value="<?php echo $nom;?>"> </td>
       </tr>
     <tr>
         <tr>
        <td> <label> Description</label> </td>
        <td> <textarea class="form-control" name="Description" id ="Description" rows="4" cols="50"><?php echo $description;?></textarea>
        </td>
       </tr>
